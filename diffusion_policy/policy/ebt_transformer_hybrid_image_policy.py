...
from diffusion_policy.model.ebt.transformer_for_ebt_nlp import TransformerForEBT_NLP
...
class EBTTransformerHybridImagePolicy(BaseImagePolicy):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # Get RoboMimic config (same as DiffusionTransformerHybridImagePolicy)
        ...
        # Replace batch norm with group norm if requested
        ...
        # Replace crop randomizer for eval if requested
        if eval_fixed_crop:
            replace_submodules(
                root_module=obs_encoder,
        ...
        # Get obs feature dimension
        obs_feature_dim = obs_encoder.output_shape()[0]
        
        # Build EBT transformer
        self.model = TransformerForEBT_NLP(  # Just change the class name!
            ...
            n_head=n_head,
            n_emb=n_emb,
            p_drop_emb=p_drop_emb,        # ✓ Now supported
            p_drop_attn=p_drop_attn,      # ✓ Now supported
            causal_attn=causal_attn,      # ✓ Now supported (always True for actions in EBT)
    ...
    def get_optimizer(
        ...
        """Create optimizer (matching DiffusionTransformerHybridImagePolicy structure)."""
        # Get transformer parameter groups
    ...
    def _mcmc_step(self, actions, obs_features, step_idx, total_steps, 
        ...
        # Only detach if we don't need to keep the graph (i.e., not training or not last step)
        if create_graph:
            return actions_new  # Keep gradients for training
        else:
            return actions_new.detach()  # Detach for inference or intermediate steps`
    ...
    def compute_loss(self, batch: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        # Encode observations (same as diffusion policy)
        To = self.n_obs_steps
    ...
    def predict_action(self, obs_dict: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        # Get action chunk (receding horizon)
        start = To - 1
